...
class Database:
    ...
    async def list_ideas(
        ...
        if search:
            like = f"%{search}%"
            clauses.append(
                "(i.title LIKE ? OR i.problem LIKE ? OR i.solution LIKE ? OR i.impact LIKE ? OR i.resources LIKE ?)"
            )
            params.extend([like, like, like, like, like])

        where = f"WHERE {' AND '.join(clauses)}" if clauses else ""
        order_by = "score DESC, i.created_at DESC" if sort == "rating" else "i.created_at DESC"
        limit_sql = ""
    ...
    async def toggle_vote(self, idea_id: int, user_id: int) -> bool:
        async with aiosqlite.connect(self.path) as db:
            cursor = await db.execute("SELECT 1 FROM votes WHERE idea_id = ? AND user_id = ?", (idea_id, user_id))
        ...
        async with aiosqlite.connect(self.path) as db:
            ...
            cursor = await db.execute(
                ...
                SELECT
                    ...
                    SUM(status = 'rejected') AS rejected_ideas,
