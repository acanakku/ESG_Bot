...
async def stats(message: Message, db: Database, settings: Settings) -> None:
    ...
    await message.answer(
        ...
        f"Отклонено: {row['rejected_ideas'] or 0}\n"
...
async def moderate(callback: CallbackQuery, db: Database, settings: Settings, bot: Bot, state: FSMContext) -> None:
    ...
    try:
        await bot.send_message(idea["author_id"], f"Ваша идея #{idea_id} одобрена и опубликована в базе проектов.")
...
async def reject_with_comment(message: Message, state: FSMContext, db: Database, settings: Settings, bot: Bot) -> None:
    ...
    await db.set_status(idea_id, "rejected", message.from_user.id, comment)
    await state.clear()
    try:
        await bot.send_message(idea["author_id"], f"Ваша идея #{idea_id} отклонена.\nПричина: {comment}")
    except TelegramAPIError:
        pass
